...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            parents = set()
            prerequisites_to_be_met =  len(action.precond_neg) + len(action.precond_pos)
            for prerequisite in action.precond_pos:
                for s in self.s_levels[level]:
                    if prerequisite == s.symbol and s.is_pos:
                        prerequisites_to_be_met -= 1
                        parents.add(s)
            for prerequisite in action.precond_neg:
                for s in self.s_levels[level]:
                    if prerequisite == s.symbol and not s.is_pos:
                        prerequisites_to_be_met -= 1
                        parents.add(s)
            if prerequisites_to_be_met == 0:
                new_action = PgNode_a(action)
                new_action.parents = parents
                self.a_levels[level].append(new_action)
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level-1]:
            for effect_add in action_node.action.effect_add:
                s_found = False
                for s in self.s_levels[level]:
                    if s.symbol == effect_add and s.is_pos == True:
                        s.parents.add(action_node)
                        action_node.children.add(s)
                        s_found = True
                if not s_found:
                    s = PgNode_s(effect_add, True)
                    s.parents.add(action_node)
                    action_node.children.add(s)
                    self.s_levels[level].add(s)
            for effect_add in action_node.action.effect_rem:
                s_found = False
                for s in self.s_levels[level]:
                    if s.symbol == effect_add and s.is_pos == False:
                        s.parents.add(action_node)
                        action_node.children.add(s)
                        s_found = True
                if not s_found:
                    s = PgNode_s(effect_add, False)
                    s.parents.add(action_node)
                    action_node.children.add(s)
                    self.s_levels[level].add(s)
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_effect_a1 in node_a1.action.effect_add:
            if pos_effect_a1 in node_a2.action.precond_neg:
                return True
        for pos_effect_a2 in node_a2.action.effect_add:
            if pos_effect_a2 in node_a1.action.precond_neg:
                return True
        for neg_effect_a1 in node_a1.action.effect_rem:
            if neg_effect_a1 in node_a2.action.precond_pos:
                return True
        for neg_effect_a2 in node_a2.action.effect_rem:
            if neg_effect_a2 in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_parent in node_a1.parents:
            for a2_parent in node_a2.parents:
                if a1_parent in a2_parent.mutex:
                    return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in self.problem.goal:
            for level_num in range(len(self.s_levels)):
                achieved_goals = [goal.symbol for goal in self.s_levels[level_num] if goal.is_pos == True]
